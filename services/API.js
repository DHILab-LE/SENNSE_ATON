...
const filePath = path.join(__dirname, "../config/auth/sennse.json");
...
let BaseAPI = (app) => {
	...
	*/
	...
	app.get('/auth/sennse', (req, res) => {
		...
		// Define the JSON file path dynamically
		const jsonFilePath = path.join(__dirname, "../data/scenes", extractedPath, "sennse.json");
	...
	app.post("/delete-sensor", (req, res) => {
		...
		if (!selectedSensorValue || !selectedNodeId || !extractedPath) {
			return res.status(400).json({ error: "Missing selectedSensorValue or selectedNodeId or extractedPath" });
		}

		console.log("Delete Sensor EndPoint !!");
		// const jsonFilePath = path.join(__dirname, "../config/auth/sennse.json");
		const jsonFilePath = path.join(__dirname, `../data/scenes/${extractedPath}/sennse.json`);
		...
		fs.readFile(jsonFilePath, "utf8", (err, data) => {
			...
			const node = jsonData.deviceIds.find((device) => device.nodeId === selectedNodeId);
			if (!node) {
				return res.status(404).json({ error: "Node not found" });
			}

			// Remove the sensor from the sensors object
			const sensors = node.sematicIDs[0]; // Assuming `sensors` is always the first object in the array
			let sensorFound = false;

			for (const [key, value] of Object.entries(sensors)) {
				if (value === selectedSensorValue) {
					delete sensors[key]; // Remove the sensor
					sensorFound = true;
					break;
				}
			}
			...
			if (!sensorFound) {
				return res.status(404).json({ error: "Sensor not found in the specified node" });
			}
			...
			fs.writeFile(jsonFilePath, JSON.stringify(jsonData, null, 2), (writeErr) => {
				...
				res.json({ message: "Sensor deleted successfully", updatedNode: node });
			});
		});
	});
	...
	app.post('/save-image', (req, res) => {
		...
		const filePath = path.join(__dirname, '../public/images', 'widget.png'); // Define the save path
	...
	app.delete('/delete-image', (req, res) => {
		...
		fs.access(filePath, fs.constants.F_OK, (err) => {
			...
			fs.unlink(filePath, (err) => {
				...
				console.log('Image deleted successfully at:', filePath);
	...
	app.post("/save-json", (req, res) => {
		...
		const dirPath = path.join(__dirname, "../data/scenes", savePath);
		const filePath = path.join(dirPath, "sennse.json");
	...
	const uploadDir = path.join(__dirname, "../data/collections/samples/models");
	...
	app.post("/upload", upload.single("modelFile"), (req, res) => {		
